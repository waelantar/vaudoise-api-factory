...
/** Value Object representing a phone number in E.164 format. Immutable and self-validating. */
public record PhoneNumber(String value) {
  ...
  private void validate(String value) {
    if (value == null || value.trim().isEmpty()) {
      throw new IllegalArgumentException(
          "Invalid phone number format: Phone number cannot be empty");
    }
  ...
  public String format() {
    ...
    if (value.length() > 3) {
      return value.substring(0, 3) + " " + value.substring(3).replaceAll("(.{3})", "$1 ").trim();
    }
    return value;
  }
}
